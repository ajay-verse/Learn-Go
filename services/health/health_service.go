...
func NewService(logger *zap.Logger, mongoClient *mongo.Client, redisHandler *redis.Client) *HealthCheckerService {
...
func (h *HealthCheckerService) Health(ctx context.Context) bool {
	...
	// check redis ping
	statusCmd := h.redisClient.Ping(ctx)
	if redisPingErr := statusCmd.Err(); redisPingErr != nil {
